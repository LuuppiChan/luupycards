...
# not used for now
def load_gamemodes(gamemodes_path):
    gamemodes = {}
    for file in os.listdir(gamemodes_path):
        if file.endswith(".py") and file != "__init__.py":
            module_name = file[:-3] # removes ".py" extension
            module = importlib.import_module(f"gamemodes.{module_name}")
            gamemodes[module_name] = module
        return gamemodes
